  ...
  ngAfterViewInit() {
    ...
    this.allCells.changes.subscribe(() => {
     ...
     setCalendar (type: string) {
      ...
      if (type === 'month') {
        for (let i = -5; i < 25; i++) {
          let check = false;
          if (moment().add(i, 'day').isAfter(moment())) {
            check = true;
          }
          if (moment().add(i, 'day').isSame(moment())) {
            check = true;
          }
          this.appDate.push({
            month: moment().format('MMM'),
            day: moment().add(i, 'day').format('MMM DD ddd'),
            today: false
          }
          
        );

        this.sDate.push(moment().add(i, 'day').format('YYYY-MM-DD'));
      ...
      if (type === 'day') {
        ...
        this.sDate.push(moment().add(i, 'day').format('moment.HTML5_FMT.DATE'));
     ...
     if (type === 'month') {
       ...
       for (let i = -5; i < 25; i++) {
         let check = false;
         if (moment().add(i+offset, 'day').isAfter(moment())) {
           check = true;
         }
         if (moment().add(i + offset, 'day').isSame(moment())) {
           check = true;
         }
         this.appDate.push({
           month: moment().add(offset).format('MMM'),
           day: moment().add(i + offset, 'day').format('MMM DD ddd'),
           today: false
         }
         
       );

       this.sDate.push(moment().add(i + offset, 'day').format('YYYY-MM-DD'));
   
         this.calendar_dates.push(moment().add(i + offset, 'day').format('DD ddd'));
   
         this.boxes.push({
           date: moment().add(i + offset, 'day').format('YYYY-MM-DD'),
           state: 'default',
           value: null,
           disp: check,
           today: false
     ...
     if (type === 'day') {
       ...
       this.sDate.push(moment().add(i + offset, 'day').format('moment.HTML5_FMT.DATE'));
    ...
    setTodayColor () {
      for (let day of this.appDate) {
        if (day.day === moment().format('MMM DD ddd')) {
          day.today = true;
        }
      }
      for (let box of this.boxes) {
        if ( box.date === moment().format('YYYY-MM-DD')) {
          box.today = true;
      }
    }
    }
